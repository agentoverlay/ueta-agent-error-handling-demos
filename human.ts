...
// Endpoint to revert a flagged order (called via AJAX from the dashboard).
app.post("/revert", async (req, res) => {
    const { orderId } = req.body;
    if (!orderId) {
        return res.status(400).json({ error: "orderId is required" });
    }
    // Forward the revert request to the business API.
    try {
...
app.get("/dashboard", (req, res) => {
    ...
    <html>
    <head>
      <title>Flagged Orders Dashboard</title>
      <style>
  ...
  `;
    if (flaggedOrders.length === 0) {
        ...
        <tr id="row-${order.id}">
          ...
          <td>${order.error || ""}</td>
          <td>
            <button onclick="revertOrder('${order.id}')">Revert</button>
          </td>
        </tr>
      `;
        });
    ...
    html += `
      ...
      <script>
        async function revertOrder(orderId) {
          if (!confirm('Are you sure you want to revert order ' + orderId + '?')) {
            return;
          }
          try {
...
// Use an environment variable to configure the human service port (default 5002).
