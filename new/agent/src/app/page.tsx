  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'products':
        return <ProductList />;
      case 'order':
        return <CreatePurchaseOrder onAccountUpdated={setAccount} currentAccount={account} />;
      case 'dashboard':
      ...
      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">UETA Agent Dashboard</h1>
            ...
            {account && (
              <div className="bg-blue-50 p-3 rounded-md">
                <p className="text-sm text-gray-600">Account ID: <span className="font-medium">{account.id.substring(0, 8)}...</span></p>
                <p className="text-sm text-gray-600">Balance: <span className="font-medium text-green-600">${account.wallet.toFixed(2)}</span></p>
              </div>
            )}
          </div>
        </div>
