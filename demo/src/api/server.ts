...
function initializeMetrics() {
    try {
        ...
        if (fs.existsSync(orderMetaFile)) {
            ...
            const pendingOrders = Object.values(orderMeta).filter(
                (order: any) => order.status === "pending_confirmation",
            );
...
app.get("/api/account", (req, res) => {
    try {
        if (!fs.existsSync(ACCOUNT_FILE)) {
            return res
                .status(404)
                .json({ error: "Account not found. Create an account first." });
        }
        ...
        return res
            .status(500)
            .json({ error: `Error getting account: ${error}` });
    }
});
...
app.post("/api/account", (req, res) => {
    try {
        ...
        return res
            .status(500)
            .json({ error: `Error creating account: ${error}` });
    }
});
...
app.get("/api/products", async (req, res) => {
    try {
        ...
        return res
            .status(500)
            .json({ error: `Error connecting to business API: ${error}` });
    }
});
    ...
    try {
        ...
        try {
            if (fs.existsSync(orderMetaFile)) {
                ...
                localPendingOrders = Object.values(orderMeta).filter(
                    (order: any) =>
        ...
        const allPendingOrders = [
            ...localPendingOrders.map((order: any) => ({
                ...order,
                source: "agent",
            })),
            ...sellerPendingOrders,
        ...
        return res
            .status(500)
            .json({ error: `Error fetching pending orders: ${error}` });
    }
});
...
app.get("/api/policies", (req, res) => {
    try {
        ...
        return res
            .status(500)
            .json({ error: `Error getting policies: ${error}` });
    }
});
...
app.get("/api/policies/:id", (req, res) => {
    try {
        ...
        return res
            .status(500)
            .json({ error: `Error getting policy: ${error}` });
    }
});
...
app.post("/api/policies", (req, res) => {
    try {
        const { name, description, target, operator, value, enabled } =
            req.body;
        ...
        if (!Object.values(PolicyTarget).includes(target as PolicyTarget)) {
            return res.status(400).json({
                error: `Invalid target. Must be one of: ${Object.values(
                    PolicyTarget,
                ).join(", ")}`,
            });
        }
        ...
        // Validate operator
        if (
            !Object.values(PolicyOperator).includes(operator as PolicyOperator)
        ) {
            return res.status(400).json({
                error: `Invalid operator. Must be one of: ${Object.values(
                    PolicyOperator,
                ).join(", ")}`,
            });
        }
        ...
        return res
            .status(500)
            .json({ error: `Error creating policy: ${error}` });
    }
});
...
app.put("/api/policies/:id", (req, res) => {
    try {
        ...
        if (
            target &&
            !Object.values(PolicyTarget).includes(target as PolicyTarget)
        ) {
            return res.status(400).json({
                error: `Invalid target. Must be one of: ${Object.values(
                    PolicyTarget,
        ...
        return res
            .status(500)
            .json({ error: `Error updating policy: ${error}` });
    }
});
...
app.delete("/api/policies/:id", (req, res) => {
    try {
        ...
        return res
            .status(500)
            .json({ error: `Error deleting policy: ${error}` });
    }
});
...
app.post("/api/policies/check", (req, res) => {
    try {
        const { sku, quantity, totalPrice, walletBalance, isAgentTransaction } =
            req.body;
        ...
        return res
            .status(500)
            .json({ error: `Error checking policies: ${error}` });
    }
});
...
app.get("/api/logs", (req, res) => {
    try {
        ...
        return res
            .status(500)
            .json({ error: `Error reading transaction logs: ${error}` });
    }
});
...
app.post("/api/ueta-add-funds", (req, res) => {
    try {
        ...
        agentAuditLog(
            `Added funds to wallet: ${amount.toFixed(
                2,
            )}. New balance: ${account.wallet.toFixed(2)}`,
        );
...
app.post("/api/ueta-withdraw-funds", (req, res) => {
    try {
        ...
        if (account.wallet < amount) {
            return res.status(400).json({
                error: `Insufficient funds. Current balance: ${account.wallet.toFixed(
                    2,
                )}`,
            });
        }
        ...
        agentAuditLog(
            `Withdrew funds from wallet: ${amount.toFixed(
                2,
            )}. New balance: ${account.wallet.toFixed(2)}`,
        );
        ...
        return res
            .status(500)
            .json({ error: `Error withdrawing funds: ${error}` });
    }
});
...
app.post("/api/order", async (req, res) => {
    ...
    try {
        ...
        if (!sku || !quantity) {
            return res
                .status(400)
                .json({ error: "SKU and quantity are required." });
        }
        ...
        const enabledPolicies = PolicyService.loadPolicies().filter(
            (p) => p.enabled,
        );
        ...
        if (simulateError) {
            agentAuditLog(
                `Error simulation active: Multiplying quantity from ${quantity} to ${actualQuantity}`,
            );
            console.log(
                `[API] Error simulation active! Multiplying quantity from ${quantity} to ${actualQuantity}`,
            );
        }
        ...
        console.log(
            `[API] Policy check result: requires approval = ${policyResult.requiresApproval}`,
        );
        ...
        if (policyResult.requiresApproval) {
            const triggerReasons = policyResult.evaluations
                ...
                .join("; ");
            agentAuditLog(
                `Order requires approval due to policies: ${triggerReasons}`,
            );
            ...
            try {
                const orderMetaFile = path.join(
                    __dirname,
                    "../data/order_meta.json",
                );
                let orderMeta = {};
                ...
                agentAuditLog(
                    `Stored pending order awaiting approval: ${pendingOrder.id}`,
                );
            } catch (err) {
            ...
            return res.json({
                ...
                policyEvaluations: policyResult.evaluations.filter(
                    (e) => e.triggered,
                ),
            });
        }
        ...
        agentAuditLog(
            `Order placed: ${JSON.stringify(order)}, policy triggered: ${
        ...
        return res.json({
            ...
            policyEvaluations: policyResult.requiresApproval
                ? policyResult.evaluations
                : undefined,
        });
    } catch (error) {
    ...
    try {
        ...
        if (localOrder && localOrder.status === "pending_confirmation") {
            ...
            if (!orderResponse.ok) {
                const errorData = await orderResponse.json();
                agentAuditLog(
                    `Error sending approved order to seller: ${JSON.stringify(
                        errorData,
                    )}`,
                );
                return res.status(orderResponse.status).json(errorData);
            ...
            console.log(
                `Order approved and sent to seller: ${orderId} -> ${sellerOrder.id}. Updated wallet balance: ${account.wallet}`,
            );
            ...
            agentAuditLog(
                `Approved and sent order to seller: ${orderId} -> ${sellerOrder.id}`,
            );
...
mcpServer.resource("account", "account://info", async (uri) => {
    if (!fs.existsSync(ACCOUNT_FILE)) {
        return {
            contents: [
                {
...
mcpServer.resource("products", "products://list", async (uri) => {
    try {
        const response = await fetch(`${config.sellerUrl}/products`);
        if (!response.ok) {
            agentAuditLog("Error fetching products from business API.");
...
mcpServer.resource("pending", "pending://orders", async (uri) => {
    try {
        if (!fs.existsSync(ACCOUNT_FILE)) {
            return {
                contents: [
...
mcpServer.resource("agent", "agent://status", async (uri) => {
    if (!fs.existsSync(ACCOUNT_FILE)) {
        return {
            contents: [
                {
            ...
            if (!response.ok) {
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error placing order: ${JSON.stringify(
                                errorData,
                            )}`,
                        },
                    ],
            ...
            if (!response.ok) {
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error approving order: ${JSON.stringify(
                                errorData,
                            )}`,
                        },
                    ],
...
mcpServer.tool("start-autonomous-agent", {}, async () => {
    if (!fs.existsSync(ACCOUNT_FILE)) {
        agentAuditLog("Agent failed to start: No account found.");
        return {
            content: [
...
mcpServer.tool("stop-autonomous-agent", {}, async () => {
    if (!agentRunning) {
        return {
            content: [
                {
