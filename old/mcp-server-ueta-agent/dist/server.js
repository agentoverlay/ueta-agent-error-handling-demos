...
function agentAuditLog(message) {
...
function addTransaction(description, amount, type) {
    ...
    agentAuditLog(`Transaction recorded: ${description}, Amount: ${amount}, Type: ${type}`);
}
...
server.resource("products", "products://list", async (uri) => {
    try {
        const response = await (0, node_fetch_1.default)("http://localhost:4000/products");
        if (!response.ok) {
            ...
            return {
                contents: [
                    {
                        uri: uri.href,
                        text: JSON.stringify(fallbackProducts, null, 2) +
...
server.resource("pending", "pending://orders", async (uri) => {
    try {
        ...
        const myPending = inMemoryOrders.filter((order) => order.accountId === account.id &&
...
server.resource("stats", "stats://overall", async (uri) => {
    try {
        const stats = {
            ...
            totalAmountPaid: inMemoryOrders.reduce((sum, order) => sum + order.totalPrice, 0),
            pendingOrders: inMemoryOrders.filter((order) => order.status === "pending" ||
                order.status === "pending_confirmation").length,
            completedOrders: inMemoryOrders.filter((order) => order.status === "delivered" || order.status === "approved").length,
        };
        return {
            contents: [
                {
        ...
        contents: [
            {
                ...
                text: JSON.stringify({
                    ...
                    totalOrders: inMemoryOrders.filter((o) => o.accountId === account.id).length,
                }, null, 2),
            },
        ],
    };
...
server.tool("ueta-create-account", { deposit: zod_1.z.number().default(100) }, async ({ deposit }) => {
...
server.tool("ueta-check-balance", {}, async () => {
    ...
    return {
        content: [
            {
                type: "text",
                text: `Current wallet balance: ${account.wallet}\n\nAccount ID: ${account.id}\n\nRecent transactions:\n${recentTransactions
                    .map((t) => `${t.timestamp.substring(0, 19)} | ${t.description.padEnd(30)} | ${t.type.padEnd(10)} | ${t.amount}`)
                    .join("\n")}`,
            },
        ],
    };
...
server.tool("ueta-add-funds", { amount: zod_1.z.number().positive() }, async ({ amount }) => {
    const account = getAccount();
    if (!account) {
        return {
            content: [
    ...
    agentAuditLog(`Added ${amount} to wallet. New balance: ${account.wallet}`);
    return {
        content: [
            {
                type: "text",
...
server.tool("ueta-withdraw-funds", { amount: zod_1.z.number().positive() }, async ({ amount }) => {
    const account = getAccount();
    if (!account) {
        return {
            content: [
    ...
    agentAuditLog(`Withdrew ${amount} from wallet. New balance: ${account.wallet}`);
    return {
        content: [
            {
                type: "text",
...
server.tool("ueta-transaction-stats", {}, async () => {
    ...
    const withdrawals = inMemoryTransactions.filter((t) => t.type === "withdrawal");
...
server.tool("ueta-get-pending-orders", {}, async () => {
    ...
    // Get pending orders
    const pendingOrders = inMemoryOrders.filter((order) => order.accountId === account.id &&
        (order.status === "pending" ||
            order.status === "pending_confirmation"));
    ...
    return {
        content: [
            {
                type: "text",
                text: `You have ${pendingOrders.length} pending orders:\n\n` +
                    pendingOrders
...
// Tool: Place an order
server.tool("ueta-place-order", {
    sku: zod_1.z.string(),
    quantity: zod_1.z.number().int().positive(),
    agentMode: zod_1.z.boolean().default(false),
    ...
    try {
        ...
        const res = await (0, node_fetch_1.default)("http://localhost:4000/products");
        let products;
        if (!res.ok) {
            agentAuditLog("Error fetching products, using fallback products.");
        ...
        try {
            const response = await (0, node_fetch_1.default)("http://localhost:4000/order", {
                method: "POST",
            ...
            if (response.ok) {
                ...
                useAPIOrder = true;
                agentAuditLog(`Order placed via API: ${JSON.stringify(orderFromAPI)}`);
            }
        }
        catch (error) {
            agentAuditLog(`Error connecting to order API: ${error}. Using internal order processing.`);
        }
        ...
        if (!useAPIOrder) {
            ...
            agentAuditLog(`Order created internally: ${JSON.stringify(order)}`);
        }
        // Update wallet balance
        account.wallet -= totalCost;
        saveAccount(account);
        // Record purchase transaction
        addTransaction(`Purchased ${quantity}x ${product.description || sku}`, totalCost, "purchase");
...
// Tool: Approve order
server.tool("ueta-approve-order", { orderId: zod_1.z.string() }, async ({ orderId }) => {
    try {
        // Try external API first
        let approved = false;
        try {
            const response = await (0, node_fetch_1.default)("http://localhost:4000/approve", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
            ...
            agentAuditLog(`Error connecting to approve API: ${error}. Using internal approval.`);
        }
        // Use internal approval if API call failed
        if (!approved) {
            // Find the order in memory
            const orderIndex = inMemoryOrders.findIndex((order) => order.id === orderId);
            if (orderIndex === -1) {
                return {
                    content: [
                        {
            ...
            while (agentRunning) {
                ...
                try {
                    ...
                    try {
                        ...
                        agentAuditLog("Agent error: Failed to fetch products from API, using fallback.");
                        products = [
                            {
                                sku: "SKU001",
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            orderPlaced = true;
                            agentAuditLog(`Agent order placed via API: ${JSON.stringify(order)}`);
                            // Update wallet
                            currentAccount.wallet -= totalCost;
                            saveAccount(currentAccount);
                            // Record purchase transaction
                            addTransaction(`Agent purchased ${quantity}x ${product.description || product.sku}`, totalCost, "purchase");
                            agentAuditLog(`Agent updated wallet balance: ${currentAccount.wallet}`);
                        }
                    }
                    catch (error) {
                        agentAuditLog(`Agent error connecting to order API: ${error}`);
                    }
                    ...
                    if (!orderPlaced) {
                        ...
                        inMemoryOrders.push(order);
                        agentAuditLog(`Agent order placed internally: ${JSON.stringify(order)}`);
                        // Update wallet
                        currentAccount.wallet -= totalCost;
                        saveAccount(currentAccount);
                        // Record purchase transaction
...
server.tool("ueta-get-dashboard-url", { port: zod_1.z.number().default(6001) }, async ({ port }) => {
    return {
        content: [
            {
                type: "text",
...
server.tool("ueta-list-products", {}, async () => {
    ...
    try {
        const response = await (0, node_fetch_1.default)("http://localhost:4000/products");
        if (!response.ok) {
            agentAuditLog("ueta-list-products: Error fetching products from business API, using fallback.");
            const fallbackProducts = [
                { sku: "SKU001", description: "Widget A", price: 50 },
                { sku: "SKU002", description: "Widget B", price: 30 },
                { sku: "SKU003", description: "Gadget C", price: 100 },
        ...
        agentAuditLog(`ueta-list-products: Failed to connect to products API: ${error}`);
        const fallbackProducts = [
            { sku: "SKU001", description: "Widget A", price: 50 },
            { sku: "SKU002", description: "Widget B", price: 30 },
            { sku: "SKU003", description: "Gadget C", price: 100 },
...
// 3) Tool: Revoke Order
server.tool("ueta-revoke-order", { orderId: zod_1.z.string() }, async ({ orderId }) => {
    const account = getAccount();
    if (!account) {
        return {
            content: [
    ...
    // Find the order
    const orderIndex = inMemoryOrders.findIndex((order) => order.id === orderId && order.accountId === account.id);
    if (orderIndex === -1) {
        return {
            content: [
                {
